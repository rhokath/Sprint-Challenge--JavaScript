...
// })
let uniList = graduates.forEach((uni) => universities.push(uni.university));
...
let contactList = graduates.forEach((info)=> contactInfo.push(info.first_name + ' ' + info.email))
...
const uni = graduates.filter((name) => name.university.includes("Uni"));
...
// })
let displayNames = zooAnimals.forEach((name) => animalNames.push(`Name: ${name.animal_name} Scientific: ${name.scientific_name}`))
